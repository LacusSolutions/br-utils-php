...
| # | Artifact | Harness |
|---|----------|---------|
| 1 | Source (`src/`) changes + `Exceptions/` | [`agents/package-arch.md`](package-arch.md) |
| 2 | PHPDoc on all changed/new symbols | [`agents/phpdoc.md`](phpdoc.md) |
| 3 | Behavior unit tests | [`agents/unit-tests.md`](unit-tests.md) |
| 4 | README update (options table, usage example) | [`agents/readme-docs.md`](readme-docs.md) |
| 5 | CHANGELOG entry | [`agents/changelogs.md`](changelogs.md) |
| 6 | `composer.json` `"require"` / `"autoload"` (if changed) | [`agents/dependencies.md`](dependencies.md) |
| 7 | Domain parity check (if `cpf-*` / `cnpj-*`) | [`agents/domain-parity.md`](domain-parity.md) |
| 8 | Aggregator cascade (if sub-package changed) | [`agents/aggregator-package.md`](aggregator-package.md) |
...
If the aggregator re-export does not yet expose the new symbol, add it to the `src/` entry point of that aggregator. See [`agents/aggregator-package.md`](aggregator-package.md).
...
- [ ] PHPDoc updated on all changed symbols per [`agents/phpdoc.md`](phpdoc.md)
